...
<div class="flex flex-col h-full"
                 ...
                 if (delay > 0 && delay < 86400000) {
                     setTimeout(() => {
                         if (Notification.permission === 'granted') {
                             new Notification('⏰ MyNotepad Reminder', {
                                 body: r.title,
                                 icon: '/favicon.ico',
                             });
                         }
    ...
    <div class="px-5 py-4 bg-zinc-50/50 dark:bg-zinc-950/20 border-b border-zinc-200 dark:border-zinc-800 flex flex-col gap-3">
    ...
    <div x-show="showAddForm"
         ...
         class="px-5 py-4 border-b border-zinc-200 dark:border-zinc-800 bg-brand-green/5"
         style="display:none;">
        <form wire:submit.prevent="addTodo" class="space-y-3">
            ...
            <div class="flex gap-2">
                ...
                @foreach(['low' => ['🟢','brand-green'], 'medium' => ['🟡','brand-yellow'], 'high' => ['🔴','brand-red']] as $p => [$emoji, $color])
                    <label class="flex items-center gap-1 cursor-pointer rounded-lg px-3 py-1.5 text-xs font-bold border transition-all
            ...
            <div class="grid grid-cols-2 gap-2">
                <div>
                    <label class="block text-[10px] font-bold text-zinc-400 uppercase tracking-wide mb-1">Due date</label>
                ...
                <div>
                    <label class="block text-[10px] font-bold text-zinc-400 uppercase tracking-wide mb-1">🔔 Remind me at</label>
    ...
    <div class="flex-1 overflow-y-auto divide-y divide-zinc-100 dark:divide-zinc-800/50">
        @forelse($todos as $todo)
            <div class="group p-4 transition-all hover:bg-zinc-50 dark:hover:bg-zinc-900/40
                ...
                @if($editId === $todo->id)
                    ...
                    <form wire:submit.prevent="saveEdit" class="space-y-2">
                        ...
                        <div class="flex gap-1">
                            @foreach(['low' => ['🟢','brand-green'], 'medium' => ['🟡','brand-yellow'], 'high' => ['🔴','brand-red']] as $p => [$emoji, $color])
                                <label class="flex items-center gap-1 cursor-pointer rounded-lg px-2 py-1 text-[11px] font-bold border transition-all
                                              {{ $editPriority === $p ? "border-{$color} bg-{$color}/10 text-{$color}" : 'border-zinc-200 dark:border-zinc-700 text-zinc-500' }}">
                                    <input type="radio" wire:model="editPriority" value="{{ $p }}" class="sr-only" />
                                    {{ $emoji }} {{ ucfirst($p) }}
                                </label>
                            @endforeach
                        </div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex flex-wrap items-center gap-1.5 mt-1.5">
                                ...
                                @endphp
                                <span class="inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-bold {{ $pClass }}">
                                ...
                                @if($todo->reminder_at && !$todo->is_done)
                                    <span class="inline-flex items-center gap-1 rounded-full border border-brand-blue/25 bg-brand-blue/10 px-2 py-0.5 text-[10px] font-semibold text-brand-blue">
